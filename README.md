...
You send a message on Telegram. The ESP32-S3 picks it up over WiFi, feeds it into an agent loop — the LLM thinks, calls tools, reads memory — and sends the reply back. Supports **Anthropic (Claude)**, **OpenAI (GPT)**, and **Google Gemini** as providers, switchable at runtime. Everything runs on a single $5 chip with all your data stored locally on flash.
...
- An **Anthropic API key** — from [console.anthropic.com](https://console.anthropic.com), or an **OpenAI API key** — from [platform.openai.com](https://platform.openai.com), or a **Google Gemini API key** — from [aistudio.google.com](https://aistudio.google.com)
...
mimi> set_api_key sk-ant-api03-... # change API key (Anthropic, OpenAI, or Gemini)
mimi> set_model_provider openai    # switch provider (anthropic|openai|gemini)
...
MimiClaw supports tool calling for Anthropic, OpenAI, and Google Gemini — the LLM can call tools during a conversation and loop until the task is done (ReAct pattern).
...
- **Multi-provider** — supports Anthropic (Claude), OpenAI (GPT), and Google Gemini, switchable at runtime
